...
class ReservationController {
    ...
    async getReservationsByAreaAndDate(req, res) {
        try {
            const { areaId, fecha } = req.params;
            
            if (!areaId || !fecha) {
                return res.status(400).json({
                    success: false,
                    message: 'areaId y fecha son requeridos'
                });
            }

            const reservas = await this.reservationService.getReservationsByAreaAndDate(
                parseInt(areaId), 
                fecha
            );
